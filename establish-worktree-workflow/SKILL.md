...
- Read [cleanup.md](references/cleanup.md) before designing, changing, or verifying worktree cleanup and recovery capabilities.
...
7. **Use supported commands.** Perform dev, service, test, cleanup/removal, and integration operations only through project-supported entrypoints. Route every worktree cleanup, retry, and explicitly authorized legacy-residual adoption through one project-native cleanup operation. Renew the applicable lease immediately before each command that changes tracked files, the index, commits, branches, or worktrees.

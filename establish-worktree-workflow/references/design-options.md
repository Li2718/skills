...
| Removed-target locks (`LOCK-10`) | Preserve canonical worktree identity in the lease; renew by record and validate selective group release before mutation | Another durable identity and atomic partial-release mechanism that does not resolve a deleted path |
...
| Cleanup recovery (`LIFE-8`, `LIFE-9`) | Atomic recovery receipt under the Git common directory, keyed by canonical target identity and retained until final verification | A durable idempotency journal or state machine shared by every worktree that proves which phases completed |
| Residual directory removal (`LIFE-10`) | Do not follow directory symlinks; make owned entries writable, retry removal with bounded backoff, then verify absence | A platform-native remover with equivalent read-only handling, sharing-violation tolerance, bounds, and diagnostics |
| Legacy adoption (`LIFE-11`) | Explicit adoption mode constrained to the configured worktree root, with registration and Git-marker rejection | A stricter project-native ownership proof that prevents arbitrary directory or branch deletion |

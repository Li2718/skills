...
- **LOCK-10 Removed-target continuity:** Renew and release a lease from its stored identity after the worktree path or Git registration disappears. Allow a token to release selected members of a group while retaining the rest. Validate ownership of every explicitly selected member before releasing any of them.
...
- **LIFE-8 Unified worktree cleanup:** Route cleanup, removal, retry, and recovery through one project-native operation. It must coordinate target validation, runtime and state cleanup, Git unregistration, directory removal, safe local-branch deletion, lease transitions, and final verification. Preserve shared durable state and adopted external resources. Reject removal of protected worktrees, branches, or active shared-state owners.
- **LIFE-9 Recoverable cleanup:** Before the first irreversible cleanup step, atomically persist non-secret recovery metadata under the Git common directory. A failed run must release every lease held by that invocation unless a documented live subprocess still owns and renews it, preserve truthful recovery state, and let the same cleanup operation resume completed phases without repeating destructive work. Remove recovery metadata only after every required cleanup outcome, including final lease release, is verified; make its removal the final state-changing cleanup step.
- **LIFE-10 Residual directory removal:** After Git unregistration, remove the target directory with cross-platform, bounded retries. Safely clear read-only attributes or permissions without following directory symlinks, back off between attempts, verify absence, and report the remaining path and last error when retries are exhausted.
- **LIFE-11 Legacy residual adoption:** Adopt an unregistered directory that predates recovery metadata only through an explicit mode authorized for that exact path. Constrain it to the project's approved worktree location, reject Git administration markers and registered worktrees, persist an adoption receipt before deletion, and do not infer or delete a branch without independent evidence.
...
- **RULE-3 Required agent behavior:** Persist session worktree reuse, the default worktree location and ignore rule, the shared-lock acquisition and renewal gate, unified recoverable cleanup, supported-entrypoint, failure-reporting, untracked-file ownership, isolated-data, test-isolation, and CI-before-fast-forward rules where the project's agents will apply them.
